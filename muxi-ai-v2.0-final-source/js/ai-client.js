...
export class AIClient{
  ...
  isUserNameQuestion(text){
    const value=this.normalizeLocalText(text);
    return /^(?:(?:你知道|你记得|还记得)\s*)?(?:我叫(?:什么|啥)(?:名字)?|我(?:是)?什么名字|我的名字(?:是|叫)?(?:什么|啥)|我的称呼(?:是)?(?:什么|啥)|你知道我的名字|你记得我的名字)(?:吗)?[呀啊呢。！!？?\s]*$/.test(value);
  }

  knownUserName(settings,memories=[]){
    const configured=this.normalizeLocalText(settings?.userName).slice(0,20);
    if(configured)return configured;

    const invalid=/^(?:什么|什么名字|啥|啥名字|谁|不知道|没有|还没有|用户)$/;
    for(const memory of Array.isArray(memories)?memories:[]){
      const content=this.normalizeLocalText(memory?.content);
      const match=content.match(/^(?:我的称呼是|我的名字是|我叫|以后叫我)\s*([^，,。！？!?]{1,20})/);
      const candidate=match?.[1]?.trim();
      if(candidate&&!invalid.test(candidate)&&!/(?:什么|啥|谁|吗|呢)$/.test(candidate))return candidate;
    }
    return"";
  }
  ...
  detectLocalIntent(text){
    ...
    if(this.isUserNameQuestion(value))return"userNameRecall";
  ...
  localReply(text,memories,settings,messages=[]){
    ...
    if(this.isUserNameQuestion(value)){
      const knownName=this.knownUserName(settings,memories);
      if(knownName)return choose([`你叫${knownName}。`,`我记得，你的名字是${knownName}。`,`当然记得，你是${knownName}。`]);
      return choose(["你还没有告诉我你的名字。可以直接说“我叫……”；我会记住。","我现在还不知道你的名字。告诉我“我叫……”，下次再问我就能回答。","目前的设置和长期记忆里都没有你的名字。你告诉我一次就好。"]);
    }
    if(/哪个版本|什么版本|版本号/.test(value)){
      return choose([`现在是${assistant} AI muxi-ai-v2.0-final，本地自动化版。`,`你正在使用的是${assistant} muxi-ai-v2.0-final。`,`这是 muxi-ai-v2.0-final，保留连续对话，并新增了安全的本地自动化框架。`]);
    }
