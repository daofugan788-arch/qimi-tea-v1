...
export function extractMemories(text){const value=String(text).trim(),candidates=[];const invalidName=/^(?:什么|什么名字|啥|啥名字|谁|不知道|没有|还没有|用户)(?:吗|呢)?$/;const rules=[{regex:/(?:请)?记住(?:一下)?[，,：:\s]*(.{2,100})/i,type:"note",format:v=>v},{regex:/(?:我叫|以后叫我)[\s：:]*([^，,。！？!?]{1,20})/i,type:"identity",format:v=>`我的称呼是${v}`,validate:v=>!invalidName.test(v)&&!/(?:什么|啥|谁|吗|呢)$/.test(v)},{regex:/我不喜欢[\s：:]*(.{1,60})/i,type:"preference",format:v=>`我不喜欢${v}`,validate:v=>!/(?:什么|啥|哪(?:个|些)?)(?:吗|呢)?$/.test(v)},{regex:/我喜欢[\s：:]*(.{1,60})/i,type:"preference",format:v=>`我喜欢${v}`,validate:v=>!/(?:什么|啥|哪(?:个|些)?)(?:吗|呢)?$/.test(v)}];for(const rule of rules){const match=value.match(rule.regex);if(match?.[1]){const captured=match[1].replace(/[。！？!?]+$/g,"").trim();if(captured&&(!rule.validate||rule.validate(captured)))candidates.push({content:rule.format(captured),type:rule.type})}}return candidates.slice(0,2)}
